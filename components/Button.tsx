...
import { decrementCartItem, getQuantity, incrementCartItem } from "@/actions/action.cart";
import { useEffect, useState } from "react";

export default function Button({ children, disabled, productId, initialquantity }: { children: any, disabled: any, productId: string, initialquantity: number }) {
    ...
    return (
      <div className="inline-flex items-center h-11 rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <button className="h-11 w-11 grid place-items-center text-lg font-semibold text-gray-900 hover:bg-gray-50 active:bg-gray-100 transition"
          onClick={() => { setCount((c) => c + 1); incrementCartItem(productId) }}>+</button>
        <p className="w-12 text-center text-sm font-medium text-gray-900 select-none">{count}</p>
        <button className="h-11 w-11 grid place-items-center text-lg font-semibold text-gray-900 hover:bg-gray-50 active:bg-gray-100 transition"
          onClick={() => { setCount((c) => c - 1); decrementCartItem(productId, initialquantity) }}>-</button>
      </div>
    );
  }
  ...
  if (count == 0) {
    return (
      <button
        onClick={() => { setCount((c) => (c + 1)); incrementCartItem(productId) }}
        disabled={disabled}
